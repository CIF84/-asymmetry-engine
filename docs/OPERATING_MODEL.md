...
It is deliberately lighter than a software architecture. It records how the project should be governed and operated based on evidence accumulated through Experiment 044. This living model remains directional and evidence-earned: later results change current policy only when their evidence justifies the change.
...
This lifecycle is a conceptual heuristic, not a mandatory pipeline. Not every candidate will traverse every state. Cheap rejection is desirable when a necessary condition fails.

Actor-facing experiments use a non-rigid evidence ledger rather than treating `ACCESSIBLE INTERVENTION → OBSERVED DECISION EFFECT` as one proven transition. Relevant states may include:
...
Each experiment selects only the states required to establish or falsify its hypothesis; not every field is mandatory. Later evidence may establish an earlier state indirectly—for example, a substantive response can establish exposure without a platform view receipt. Preserve the distinctions: surface access is not intervention permission, delivery/publication, or actor exposure; semantic engagement is not verified human cognitive change; decision-state refinement is not observed downstream action; observed action is not economic effect; and economic effect is not value capture.
...
An actor-facing experiment should normally preregister, proportionately:
...
When the hypothesis requires them, it should also identify semantic-engagement evidence, decision-authority provenance, authorship provenance, decision-state effect, stated next action, observed downstream action, economic effect, or value capture. Exposure observability must be considered before publication: a successful post without an observable exposure path may be measurement-invalid for a behavioral hypothesis.
...
For actor-facing evidence, keep these constructs separate:
...
AI assistance or delegated-agent participation does not itself grant or remove authority. Consequential authorization continues to depend on the authority and consequence of the action, with human consent and accountability preserved where applicable, not merely on whether text was generated by a human or AI.

Where a hypothesis requires provenance, use provisional manual authorship labels `HUMAN VERIFIED`, `AI-ASSISTED DISCLOSED`, `DELEGATED AGENT DISCLOSED`, `AUTOMATED DISCLOSED`, or `UNKNOWN`, and decision-authority labels `DIRECT`, `STRONG`, `INFERRED`, or `UNKNOWN`. Never infer authorship from writing style; `UNKNOWN` is neutral and authorship is orthogonal to channel authority. Classify authority relative to the specific decision object—proposal ownership does not by itself prove organizational, implementation, budget, or final-approval authority—and do not mechanize these labels.
...
- What establishes that an actor-facing intervention was delivered?
- What establishes exposure, and can a substantive response establish it when platform view evidence is absent?
- Is the response semantically relevant to the target claim?
- What authority does the responding account or channel have over this decision object?
- What is known about message authorship, and what remains `UNKNOWN`?
- Is the evidence decision-state refinement, stated intent, observed downstream action, economic effect, or value capture?

A public or semantic-looking response must not automatically be converted into verified human cognition or economic effect.
...
- was actor exposure observable, or only delivery?
- could silence represent unseen delivery?
- does the response demonstrate semantic engagement with the actual proposition?
- are authorship, account/channel identity, authority, and accountability being conflated?
...
Synthetic-evidence contamination is a proportional validity question when a claimed effect requires evidence beyond semantic exchange; it is not a presumption that bots or automation are present.
...
Where meaningful for an actor-facing hypothesis, record the relevant manual evidence states: delivery, exposure, semantic engagement, authority provenance, authorship provenance, decision-state refinement, stated next action, observed downstream action, economic effect, and value capture. Missing or unavailable states remain `UNKNOWN`; not every state is required for every experiment, and these states must not be collapsed into a synthetic score.
